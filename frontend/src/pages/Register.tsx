...
export default function RegisterPage() {
  ...
  const fieldClass = "flex h-10 w-full rounded-lg border border-white/10 bg-white/5 px-3 py-1 text-sm text-white placeholder:text-slate-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500";
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 p-4">
      ...
      <div className="relative w-full max-w-md">
        <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 p-8 shadow-2xl">
          ...
          <p className="text-slate-400 text-sm mb-6">Start invoicing in minutes — free to try</p>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            {[
              ...
              { key: "businessName", label: "Business Name", type: "text", placeholder: "Sharma Enterprises" },
              { key: "email", label: "Email", type: "email", placeholder: "rahul@sharma.com" },
              ...
              <div key={key} className="space-y-1.5">
                ...
                {errors[key as keyof FormData] && (
                  <p className="text-xs text-rose-400">{errors[key as keyof FormData]?.message}</p>
                )}
              </div>
            ))}
            ...
            <button
              ...
              className="w-full h-10 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg font-medium transition-colors disabled:opacity-60 flex items-center justify-center gap-2 mt-2"
            >
